// Package internal with handling of commands for updating openzwave nodes
package internal
...
func (app *OpenZWaveApp) AddZWaveNode(startStop bool) {
	logrus.Infof("OpenZWaveAdapter.AddZWaveNode")
	if startStop == true {
...
func (app *OpenZWaveApp) RemoveZWaveNode(startStop bool) {
	logrus.Infof("OpenZWaveAdapter.RemoveZWaveNode")
	if startStop == true {
...
func (app *OpenZWaveApp) GetNeighbors(nodeHWID string) {
	logrus.Infof("OpenZWaveAdapter.GetNeighbors. Not supported")
	//nodeID, _ := strconv.Atoi(device.Address(true))
	//goopenzwave.GetNodeNeighbors(adapter.ozwHomeID, uint8(nodeID))
}
...
func (app *OpenZWaveApp) RemoveFailedNode(nodeHWID string) {
	logrus.Infof("OpenZWaveAdapter.RemovefailedNode: Node %s", nodeHWID)
	zwNodeID, _ := strconv.Atoi(nodeHWID)
...
func (app *OpenZWaveApp) UpdateNeighbors(nodeHWID string) {
	logrus.Infof("OpenZWaveAdapter.UpdateNeighbors: Node %s", nodeHWID)
	zwNodeID, _ := strconv.Atoi(nodeHWID)
...
func (app *OpenZWaveApp) SwitchOnOff(nodeHWID string, input *types.InputDiscoveryMessage, newValue string) {
	...
	logrus.Infof("OpenZWaveAdapter.SwitchOnOff. Device %s: Property %s: current value=%s. new value=%s, changing to: %t",
		nodeHWID, input.InputType, currentValue, newValue, onoff)

	err = goopenzwave.SetValueBool(app.ozwHomeID, valueID, onoff)
	if err != nil {
		logrus.Warnf("OpenZWaveAdapter.SwitchOnOff: Node %s: Property %s. Error: %v", nodeHWID, input.InputType, err)
	}
}

// HandleCommand for openzwave node
// Currently very basic. Only switch status is supported.
// TODO: commands for healing the network, querying neighbors, managing routes and associations.
// func (app *OpenZWaveApp) HandleCommand(nodeHWID string, input *types.InputDiscoveryMessage, payload []byte) {
func (app *OpenZWaveApp) HandleCommand(input *types.InputDiscoveryMessage, sender string, payloadStr string) {
	valueID := app.valueIDByInputID[input.InputID]
	if valueID == 0 {
		// This is not a known openzwave sensor, check for button commands
		if input == nil {
			logrus.Warn("OpenZWaveAdapter.HandleCommand. Command for input but input is not yet discovered in Zwave")
			return
		}
		...
		logrus.Infof("OpenZWaveAdapter.HandleCommand: PushButton '%s'. Value=%v", input.Instance, payloadStr)
		if input.Instance == ButtonInstanceHealNetwork {
			...
			logrus.Warningf("OpenZWaveAdapter.HandleCommand: PushButton '%s' is not a known command. Ignored.",
				input.Instance)
		}
		return
	}
	...
	default:
		logrus.Warningf("OpenZWaveAdapter.HandleCommand. Device %s: Unexpected data type %s for property %s",
			input.NodeHWID, dataType, input.InputType)
	}
	if err != nil {
		logrus.Errorf("OpenZWaveAdapter.HandleCommand. Device %s failed handling command for input %s: %s",
